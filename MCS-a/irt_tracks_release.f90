...
USE irt_parameters, ONLY: max_no_of_tracks, max_length_of_track, &
                          termination_sensitivity, n_fields, minhour, minimum_size2,minimum_size
...
input_filename  ="/disk1/guoyj/MCS-tracking/iterative_raincell_tracking/GPM-225K-6hr-a-a/output/&
irt_objects_output_"//filenamestring//".txt" 
output_filename ="/disk1/guoyj/MCS-tracking/iterative_raincell_tracking/GPM-225K-6hr-a-a/output/&
irt_tracks_output_"//filenamestring//".txt"  
output2_filename ="/disk1/guoyj/MCS-tracking/iterative_raincell_tracking/GPM-225K-6hr-a-a/output/&
irt_tracks_nohead_output_"//filenamestring//".txt"   
output3_filename ="/disk1/guoyj/MCS-tracking/iterative_raincell_tracking/GPM-225K-6hr-a-a/output/&
  ...
  DO i=1,max_no_of_tracks
	...
	!	    center_of_mass_x(:,i),center_of_mass_y(:,i),velocity_x(:,i),velocity_y(:,i))
        ...
        CALL write_output(max_length_of_track,longest_track,counter,n_fields, track_length(i), &
                    ...
                    status_beginning(i),status_end(i),minhour,minimum_size,minimum_size2)
        !ENDIF
...
!IF (status_backward .NE. 1 .AND. status_forward .EQ. 1) THEN
  ...
  ELSE ! track is only one timestep long, write immediately
    ...
    CALL write_output(max_length_of_track,longest_track,counter,n_fields, 1, &
                    ...
                    xmax(:,i),ymax(:,i) ,xmin(:,i),ymin(:,i) ,velocity_x(:,i), &
                    lfl(:,i),slfl(:,i),lbl(:,i),slbl(:,i), &
                    status_beginning(i),status_end(i),minhour,minimum_size,minimum_size2)   ! write track with length 1
  ENDIF
ENDIF
...
SUBROUTINE write_output(max_length_of_track, longest_track, counter, n_fields, track_length, &
                        ...
                        lfl, slfl, lbl, slbl, &
                        status_beginning, status_end,minhour,minimum_size,minimum_size2)
...
DO j=1,track_length
  IF (totarea3(j) .GE. minimum_size2.and. totarea(j) .GE. minimum_size) THEN
    mcsornot(j)=1
  ELSE
    mcsornot(j)=0
  ENDIF
...
IF (sum(mcsornot).ge.minhour) THEN
  aaa=0
  bbb=0
  j=1
  i=1
...
DO j=1,track_length
  ...
  WRITE(30,*) counter,cell_timestep(j),cell_age1(j),cell_age2(j),cell_id(j), &
              ...
              totarea(j), totarea1(j),totarea2(j),totarea3(j),&
              prcp_mean1(j),prcp_mean2(j),prcp_mean3(j),field_mean(j,:),field_min(j,:),field_max(j,:)
ENDDO

! only track beginnings
! IF (status_beginning .EQ. 0 .AND. status_end .EQ. 0 .AND. track_length_ofmcs .GE. minhour) THEN
