...
output_filename    = filepath//'GPM-225K-6hr-a-a/output/irt_objects_output_'//filenamestring//'.txt'
mask_filename      = filepath//'GPM-225K-6hr-a-a/output/irt_objects_mask_'//filenamestring//'.srv'
coarsevel_filename = filepath//'GPM-225K-6hr-a-a/output/irt_advection_field_'//filenamestring//'.srv'
  ...
  DO iy=1, domsize_y
    DO ix=1, domsize_x
      IF (input_field(ix,iy,1).LT.threshold .AND. input_field(ix,iy,1).GT.(miss+100) .AND. .NOT. occupied(ix,iy)) THEN
        ...
        tbmin(counter_actual,n_actual)=threshold1
        ...
        !!!!!!!!!!!!!!!!!!!!!!!!changed!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        ELSEIF (field_min(counter_actual,n_actual,1) .GE. threshold1) THEN
        ! delete this cell by overwriting it with -1 in event_number
          CALL set_event_number_to_value(domsize_x,domsize_y,-1,event_number(:,:,n_actual),counter_actual, &
               xfirst(counter_actual,n_actual),xlast(counter_actual,n_actual), &
               yfirst(counter_actual,n_actual),ylast(counter_actual,n_actual))
