...
filepath = '/disk1/guoyj/MCS-tracking/iterative_raincell_tracking/'
CALL getarg(1,filenamestring)
!filenamestring1="Tb_"//filenamestring
input_filename(1) = '/disk1/data/GPM_hourly/inputsrvdata/irt_objects_input_Tb_'//filenamestring//'.srv'
input_filename(2) = '/disk1/data/GPM_hourly/inputsrvdata/irt_objects_input_'//filenamestring//'.srv'

!!not use!! DO i=0,n_fields
!!not use!!  WRITE(input_filename(i+1),'(A18,I2.2,A4)") "irt_objects_input_",i,".srv"
!!not use!! ENDDO

output_filename    = filepath//'GPM-b-221K-pre5-5000-a-a/output/irt_objects_output_'//filenamestring//'.txt'
mask_filename      = filepath//'GPM-b-221K-pre5-5000-a-a/output/irt_objects_mask_'//filenamestring//'.srv'
coarsevel_filename = filepath//'GPM-b-221K-pre5-5000-a-a/output/irt_advection_field_'//filenamestring//'.srv'
  ...
  DO iy=1, domsize_y
    DO ix=1, domsize_x
      IF (input_field(ix,iy,1).LT.threshold .AND. input_field(ix,iy,1).GT.(miss+100) .AND. .NOT. occupied(ix,iy)) THEN
        ...
        tbmin(counter_actual,n_actual)=221
        ...
        !!!!!!!!!!!!!!!!!!!!!!!!changed!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        ELSEIF (field_min(counter_actual,n_actual,1) .GE. 221) THEN
        ! delete this cell by overwriting it with -1 in event_number
          CALL set_event_number_to_value(domsize_x,domsize_y,-1,event_number(:,:,n_actual),counter_actual, &
               xfirst(counter_actual,n_actual),xlast(counter_actual,n_actual), &
               yfirst(counter_actual,n_actual),ylast(counter_actual,n_actual))
